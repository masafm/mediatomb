...
function addVideo(obj)
{
    var title = obj.title;
    obj.title = title
	.replace(/^(.*)-([0-9][0-9][0-9][0-9])([0-9][0-9])([0-9][0-9])-([0-9][0-9])([0-9][0-9])(.*?)$/, '$3/$4 $5:$6-$1')
    
    var date = title.replace(/^.*([0-9][0-9][0-9][0-9])([0-9][0-9])([0-9][0-9])-([0-9][0-9])([0-9][0-9]).*?$/, '$1-$2-$3T$4:$5:00');
    if(date.match(/^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9]$/)) {
	obj.meta[M_DATE] = date;
    }    
    var channel = title.replace(/^.*-(.*?)-([0-9][0-9][0-9][0-9])([0-9][0-9])([0-9][0-9])-([0-9][0-9])([0-9][0-9]).*?$/, '$1');

    //var chain = new Array('全てのビデオ');
    //addCdsObject(obj, createContainerChain(chain));

    //var dir = getRootPath(object_root_path, obj.location);
    var dir = getRootPath('/', obj.location);//temporary

    if (dir.length > 0)
    {
        chain = new Array('ディレクトリ', dir[dir.length-1]);
        addCdsObject(obj, createContainerChain(chain));

	// 最近追加したファイル(フォルダ別)
	chain = new Array('最近追加したビデオ', dir[dir.length-1], '昨日');
	addCdsObject(obj, createContainerChain(chain));
	chain = new Array('最近追加したビデオ', dir[dir.length-1], '3日前');
	addCdsObject(obj, createContainerChain(chain));
	chain = new Array('最近追加したビデオ', dir[dir.length-1], '1週間前');
	addCdsObject(obj, createContainerChain(chain));
	chain = new Array('最近追加したビデオ', dir[dir.length-1], '1ヶ月前');
	addCdsObject(obj, createContainerChain(chain));
    }
    // 最近追加したファイル(全て)
    chain = new Array('最近追加したビデオ', '全て', '昨日');
    addCdsObject(obj, createContainerChain(chain));
    chain = new Array('最近追加したビデオ', '全て', '3日前');
    addCdsObject(obj, createContainerChain(chain));
    chain = new Array('最近追加したビデオ', '全て', '1週間前');
    addCdsObject(obj, createContainerChain(chain));
    chain = new Array('最近追加したビデオ', '全て', '1ヶ月前');
    addCdsObject(obj, createContainerChain(chain));
